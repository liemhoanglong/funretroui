...
const Board = (props) => {
  ...
  return (
    <Row className="mb-3" key={data._id}>
      <Col>
        { edit && data._id !== edit.id ? 
          <Card className={`${data.type === 1 ? "bg-danger" : data.type === 2 ? "bg-primary" : "bg-success"} bg-gradient text-white border-0 rounded`}>
            <Card.Body>
              <div className='d-flex justify-content-between'>
                <p className='text-start'>{data.name}</p>
                <div className='hover-cursor' onClick={() => editBoard(data._id, data.name)}>
          ...
          <Card className={`${data.type === 1 ? "bg-danger" : data.type === 2 ? "bg-primary" : "bg-success"} bg-gradient text-white border-0 rounded`}>
            <Card.Body>
              <div className='d-flex justify-content-between'>
                <p className='text-start'>{data.name}</p>
                <div className='hover-cursor' onClick={() => delBoard(data._id)}>
              ...
              >
                <Form.Control value={input} className='w-100' onChange={handleChange}/>
                <div className='d-flex mt-3 justify-content-between'>
                  <Button type='submit' variant="success" size='sm'>Save</Button>
                  <Button variant="warning" size='sm' onClick={cancelBoard}>Cancel</Button>
                </div>
              </form>
            </Card.Body>
          </Card>
